...
{
	::OutputDebugString("-------------------------------------------Lajevardipour Error Handling-\n");
	char pszError[256];
...
HRESULT GameInit();
HRESULT GameLoop(int, char []);
HRESULT GameShutDown(const int, int);
HRESULT Render(int, char []);




// Globals
static char strAppname[]="Direct3D Animation";
LPDIRECT3D9 g_pD3D;
...
const int Sno=1000; //Number of Sphere

/*    	   const int IndexCont= nRings * (nSegments+1)* 2; 
			const int VertCont= (nRings+1)*(nSegments+1);  */

const int IndexCont= 360;
const int VertCont=192;
const int nRings = 15;
const int nSegments = 11;
...
void GenerateVertex(const int, double, double, double, double, int, int, int);
void GenerateIndex();
